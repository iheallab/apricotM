...
from variables import SOFA_CRITERIA_DIR, SOFA_DIR, OUTPUT_DIR, PROSP_DIR, DATA_DIR, time_window

if not os.path.exists(f"{SOFA_CRITERIA_DIR}/results"):
    os.makedirs(f"{SOFA_CRITERIA_DIR}/results")
...
sofa = pd.read_csv(
    ...
    usecols=["icustay_id", "interval", "final_state", "transition"],
)

sofa["interval"] = sofa["interval"] + 1
sofa["shift_id"] = sofa["icustay_id"].astype(str) + "_" + sofa["interval"].astype(str)

sofa.loc[(sofa["interval"] == 1), "transition"] = "stable-stable"
...
print(sofa["transition"].value_counts())
print(outcomes["transition"].value_counts())
...
sofa["final_state"] = sofa["final_state"].map(map_states)

print(sofa["final_state"].value_counts())

sofa["stable-unstable"] = sofa["final_state"]
sofa["dead"] = sofa["final_state"]
sofa["no mv-mv"] = sofa["final_state"]
sofa["no vp- vp"] = sofa["final_state"]
sofa["no crrt-crrt"] = sofa["final_state"]
...
for cohort in cohorts:
    ...
    group.to_csv(f"{SOFA_CRITERIA_DIR}/results/{cohort[0]}_pred_labels.csv", index=False)
...
for cohort in cohorts:
    ...
    group.to_csv(f"{SOFA_CRITERIA_DIR}/results/{cohort[0]}_true_labels.csv", index=False)
