...
if not os.path.exists(f"{OUTPUT_DIR}/model/apricott"):
    os.makedirs(f"{OUTPUT_DIR}/model/apricott")
...
def objective(trial):
    ...
    d_model = trial.suggest_int("d_model", 64, 512, step=2)
...
with open("%s/model/apricott/best_params.pkl" % OUTPUT_DIR, "wb") as f:
...
early_stopping = EarlyStopping(
    patience=3, verbose=True, path=f"{OUTPUT_DIR}/model/apricott/apricott_weights.pth"
)
...
torch.save(model_architecture, f"{OUTPUT_DIR}/model/apricott/apricott_architecture.pth")
