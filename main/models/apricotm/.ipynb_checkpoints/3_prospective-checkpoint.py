...
from variables import time_window, PROSP_DATA_DIR, MODEL_DIR

#%%
# Load dataset

with h5py.File("%s/dataset.h5" % PROSP_DATA_DIR, "r") as f:
    data = f["prospective"]
    X = data["X"][:]
    static = data["static"][:]
...
model.load_state_dict(
    torch.load(f"{MODEL_DIR}/apricotm_weights.pth", map_location=DEVICE)
)
