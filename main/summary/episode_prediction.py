...
models = ["sofa", "sofa_criteria", "apricotm"]

outcomes = ["dead", "stable-unstable"]

cohorts = ["int", "ext", "prosp"]

tasks = ["AUROC", "AUPRC", "AUROC (step)", "AUPRC (step)"]
...
for outcome in outcomes:
    ...
    for model in models:
        ...
        table = pd.read_csv(f"{MODEL_DIR}/{model}/episode_prediction/{outcome}_auroc_auprc.csv", index_col=["Unnamed: 0"])
    ...
    for cohort in cohorts:
        ...
        for task in tasks:
            ...
            for comparison in comp:
                ...
                sample_1 = table_comp.loc[(table_comp["cohort"] == cohort)&(table_comp["model"] == comparison), task].values[0]
    ...
    table_comp.to_csv(f"{OUTPUT_DIR}/summary/episode_prediction/{outcome}_auroc_auprc.csv")
