...
# Filter admissions by missigness of age information

print("-" * 20 + "Filtering by age presence" + "-" * 20)

age_eicu = pd.read_csv(
    "%s/eicu/patient.csv" % DATA_DIR, usecols=["patientunitstayid", "age"]
)
age_eicu = age_eicu[
    age_eicu["patientunitstayid"].isin(admissions_eicu["patientunitstayid"].unique())
]
age_eicu = age_eicu.dropna(subset=["age"])
...
age_mimic = pd.read_csv(
    "%s/mimic/icustays.csv" % DATA_DIR, usecols=["stay_id", "subject_id"]
)
age_mimic = age_mimic[age_mimic["stay_id"].isin(admissions_mimic["stay_id"].unique())]
age_mimic = age_mimic.merge(
    pd.read_csv("mimic/patients.csv", usecols=["subject_id", "anchor_age"]),
    how="left",
    on="subject_id",
)
age_mimic = age_mimic.dropna(subset=["anchor_age"])
...
age_uf = pd.read_csv("%s/uf/static.csv" % DATA_DIR, usecols=["icustay_id", "age"])
age_uf = age_uf[age_uf["icustay_id"].isin(admissions_uf["icustay_id"].unique())]
age_uf = age_uf.dropna(subset=["age"])
...
los_eicu = los_eicu[los_eicu["unitdischargeoffset"] >= 720]
...
los_mimic = los_mimic[los_mimic["los"] >= 0.5]
...
los_uf = los_uf[los_uf["icu_los"] >= 12]
...
admissions_eicu.to_csv("%s/final_data/admissions_eicu.csv" % DATA_DIR, index=None)
admissions_mimic.to_csv("%s/final_data/admissions_mimic.csv" % DATA_DIR, index=None)
admissions_uf.to_csv("%s/final_data/admissions_uf.csv" % DATA_DIR, index=None)
...
with open("%s/final_data/ids.pkl" % DATA_DIR, "wb") as f:
    pickle.dump(
        {
            "train": ids_train,
            "val": ids_val,
