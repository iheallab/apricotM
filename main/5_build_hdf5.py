...
seq = pd.read_csv('%s/final_data/seq.csv' % DATA_DIR)
static = pd.read_csv('%s/final_data/static.csv' % DATA_DIR)
outcomes = pd.read_csv('%s/final_data/outcomes.csv' % DATA_DIR)

outcomes = outcomes[outcomes['interval'] != 0]

seq = seq[seq['shift_id'].isin(outcomes['shift_id'].unique())]

nan_count = outcomes.isna().sum().sum()
...
y_trans = np.concatenate([y_trans[:,2:4], y_mv[:,2:], y_pressor[:,2:], y_crrt[:,2:]], axis=1)
...
with open("%s/final_data/ids.pkl" % DATA_DIR, "rb") as f:
    ids = pickle.load(f)
    ids_train, ids_val, ids_ext, ids_temp = ids["train"], ids['val'], ids['ext_test'], ids['temp_test']
...
with h5py.File("%s/final_data/dataset.h5" % DATA_DIR, "w", libver="latest") as hf:
