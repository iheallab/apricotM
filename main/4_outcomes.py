...
outcomes_eicu = pd.read_csv('%s/eicu/acuity_states_v3.csv' % DATA_DIR)
admissions_eicu = pd.read_csv('%s/final_data/admissions_eicu.csv' % DATA_DIR)

outcomes_eicu = outcomes_eicu[outcomes_eicu['patientunitstayid'].isin(admissions_eicu['patientunitstayid'])]
...
outcomes_eicu['shift_id'] = outcomes_eicu['icustay_id'].astype(str) + '_' + outcomes_eicu['interval'].astype(str)
...
outcomes_eicu.loc[(outcomes_eicu['icustay_id'].isin(dischg_ids)) & (outcomes_eicu.index == last_state_indices), 'final_state'] = 'discharge'
outcomes_eicu.loc[(outcomes_eicu['icustay_id'].isin(dead_ids)) & (outcomes_eicu.index == last_state_indices), 'final_state'] = 'dead'
...
admissions_mimic = pd.read_csv('%s/final_data/admissions_mimic.csv' % DATA_DIR)

outcomes_mimic = outcomes_mimic[outcomes_mimic['stay_id'].isin(admissions_mimic['stay_id'])]
...
outcomes_mimic['shift_id'] = outcomes_mimic['icustay_id'].astype(str) + '_' + outcomes_mimic['interval'].astype(str)
...
dead_ids['ind1'] = (pd.to_datetime(dead_ids['deathtime']) - pd.to_datetime(dead_ids['intime'])) / np.timedelta64(1, "h")

dead_ids['ind2'] = (pd.to_datetime(dead_ids['outtime']) - pd.to_datetime(dead_ids['deathtime'])) / np.timedelta64(1, "h")
...
dischg_ids = admissions_mimic.merge(dischg_ids, how='inner', on=['subject_id', 'hadm_id'])

dischg_ids['discharge'] = (pd.to_datetime(dischg_ids['dischtime']) - pd.to_datetime(dischg_ids['outtime'])) / np.timedelta64(1, 'h')
...
dischg_ids = dischg_ids[(dischg_ids['discharge'] >= -24) & (dischg_ids['discharge'] <= 24)]

print(len(dischg_ids['stay_id'].unique()))

dischg_ids = dischg_ids['stay_id'].unique()

last_state_indices = outcomes_mimic.groupby('icustay_id')['interval'].transform('idxmax')

outcomes_mimic.loc[(outcomes_mimic['icustay_id'].isin(dischg_ids)) & (outcomes_mimic.index == last_state_indices), 'final_state'] = 'discharge'
outcomes_mimic.loc[(outcomes_mimic['icustay_id'].isin(dead_ids)) & (outcomes_mimic.index == last_state_indices), 'final_state'] = 'dead'
...
admissions_uf = pd.read_csv('%s/final_data/admissions_uf.csv' % DATA_DIR)

outcomes_uf = outcomes_uf[outcomes_uf['icustay_id'].isin(admissions_uf['icustay_id'])]

print(outcomes_uf['final_state'].value_counts())

outcomes_uf['shift_id'] = outcomes_uf['icustay_id'].astype(str) + '_' + outcomes_uf['interval'].astype(str)
...
outcomes = outcomes.groupby('icustay_id').apply(transitions).reset_index()

outcomes.to_csv('%s/final_data/outcomes.csv' % DATA_DIR, index=None)
