...
with h5py.File("%s/dataset_gru.h5" % MODEL_DIR, "r") as f:
    data = f["training"]
    X_train = data["X"][:]
...
def objective(trial):
    ...
    hidden_size = trial.suggest_int("hidden_size", 8, 256, step=2)
...
early_stopping = EarlyStopping(
    patience=2, verbose=True, path=f"{MODEL_DIR}/model_weights.pth"
)
