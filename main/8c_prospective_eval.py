...
variables_redfeat = list(variable_map.values())

print(len(variables_redfeat))

seq = seq[seq["variable"].isin(variables_redfeat)]
...
from model.mamba_redfeat.mamba import ApricotM
import torch

# Load model architecture

model_architecture = torch.load(
    f"{DATA_DIR}/model/mamba_redfeat/mamba_architecture.pth"
)
...
model.load_state_dict(
    torch.load(f"{DATA_DIR}/model/mamba_redfeat/mamba_weights.pth", map_location=DEVICE)
)
...
true_labels.to_csv(
    f"{DATA_DIR}/results/mamba_tuned/redfeat/prosp_true_labels.csv", index=None
)
pred_labels.to_csv(
    f"{DATA_DIR}/results/mamba_tuned/redfeat/prosp_pred_labels.csv", index=None
)
