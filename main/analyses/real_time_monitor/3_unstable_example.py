...
from variables import MODEL_DIR, OUTPUT_DIR, PROSP_DATA_DIR
...
true_ext = pd.read_csv(f"{MODEL_DIR}/results/prosp_true_labels.csv")
pred_ext = pd.read_csv(f"{MODEL_DIR}/results/prosp_pred_labels.csv")

#%%

# Choose random unstable patient

unstable_ids = true_ext.loc[
    (true_ext["unstable"] == 1) & (pred_ext["unstable"] > 0.5), "icustay_id"
].values
unstable_examples = true_ext[true_ext["icustay_id"].isin(unstable_ids)]
stable_ids = unstable_examples.loc[
    (unstable_examples["stable-unstable"] == 1), "icustay_id"
].unique()

unstable_examples = unstable_examples[unstable_examples["icustay_id"].isin(stable_ids)]
stable_ids = unstable_examples.groupby("icustay_id").first().reset_index()
stable_ids = stable_ids.loc[
    (stable_ids["stable"] == 1), "icustay_id"
].unique()
unstable_examples = unstable_examples[unstable_examples["icustay_id"].isin(stable_ids)]

unstable_examples = unstable_examples.groupby("icustay_id").size()
unstable_examples = unstable_examples[unstable_examples >= 24]
unstable_ids = unstable_examples.index.values

print(f"Number of patients with unstable episodes: {len(unstable_ids)}")

icustay_id = np.random.choice(unstable_ids, 1)[0]
# icustay_id = 40000080
# icustay_id = 40000399
# icustay_id = 40000336
# icustay_id = 40000432
...
# Quick plot of predicted unstable score vs actual state (stable or unstable)

fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(time, pred_ext["unstable"], color="blue", label="Model prediction")
ax.plot(time, [0.2] * len(time), "red", label="0.2 Threshold")
ax.plot(time, true_ext["unstable"], "black", label="True state")
...
# Default last 4-h interval to discharge if not already assigned
if acuity["state"].iloc[-1] != "discharge":
    acuity = pd.concat([acuity, pd.DataFrame({"state": ["discharge"], "start_time": [acuity["end_time"].iloc[-1]], "end_time": [acuity["end_time"].iloc[-1] + 4]})], ignore_index=True)
...
ax2.plot(time, [0.2] * len(time), "red", label="0.2 Threshold")
ax2.fill_between(
    time,
    lower_bound[:, 2],
    upper_bound[:, 2],
...
ax3.plot(time, [0.2] * len(time), "red", label="0.2 Threshold")
ax3.fill_between(
    time,
    lower_bound[:, 3],
    upper_bound[:, 3],
...
plt.savefig(
    f"{MODEL_DIR}/examples/unstable_monitor4.png",
    format="png",
    dpi=400,
)
plt.show()
...
plt.savefig(
    f"{MODEL_DIR}/examples/unstable_vitals4.png",
    format="png",
    dpi=400,
)
plt.show()
...
print(mv["time"].tolist())
print(vp["time"].tolist())
# %%
