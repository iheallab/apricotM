...
pred_ext = pd.read_csv(f"{MODEL_DIR}/results/ext_pred_labels.csv")
...
dead_ids = true_ext.loc[
    (true_ext["discharge"] == 1) & (pred_ext["discharge"] > 0.2), "icustay_id"
].values
...
pred_ext = pred_ext[pred_ext["icustay_id"] == icustay_id].reset_index(drop=True)
...
time = list((np.arange(0, len(true_ext)) * 4))
pred = pred_ext["unstable"] + pred_ext["stable-unstable"]
...
plt.savefig(
    f"{MODEL_DIR}/examples/discharged_monitor.png",
    format="png",
    dpi=400,
)
plt.show()
...
plt.savefig(
    f"{MODEL_DIR}/examples/discharged_vitals.png",
    format="png",
    dpi=400,
)
plt.show()
