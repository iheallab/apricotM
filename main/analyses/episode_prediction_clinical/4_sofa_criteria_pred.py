...
for model in models:
    ...
    for outcome in outcomes:
        ...
        for cohort in cohorts:
            ...
            pred = recal["pred"].values
            
            true_label_step = predictions["true"].values
            pred_step = predictions["prob"].values

            n_iterations = 100
            
            prec = precision_score(true_label, pred)
            ...
            for i in range(n_iterations):
                ...
                sample_pred = pred[random_sample]
                ...
                sample_pred = pred_step[random_sample]
            ...
            table_thresh.insert(loc=0, column="Precision", value=prec)
            ...
            predictions["pred"] = predictions["prob"].values
            ...
            if len(alert_counts) > 0:
                ...
                alert = pd.Series(alert_counts).describe()
                lead = pd.Series(time_lead).describe()

                alert = f"{alert['50%']:.2f} ({alert['25%']:.2f}-{alert['75%']:.2f})"
                lead = f"{lead['50%']:.2f} ({lead['25%']:.2f}-{lead['75%']:.2f})"
