...
with h5py.File("%s/dataset_prospective.h5" % ANALYSIS_DIR, "r") as f:
    data = f["prospective"]
    X = data["X"][:]
    static = data["static"][:]
...
model_architecture = torch.load(f"{MODEL_DIR}/apricotm/model_architecture.pth")
...
model.load_state_dict(
    torch.load(f"{MODEL_DIR}/apricotm/model_weights.pth", map_location=DEVICE)
)
...
if not os.path.exists(f"{MODEL_DIR}/apricotm/results"):
    os.makedirs(f"{MODEL_DIR}/apricotm/results")

true_labels.to_csv(f"{MODEL_DIR}/apricotm/results/prosp_true_labels.csv", index=None)
pred_labels.to_csv(f"{MODEL_DIR}/apricotm/results/prosp_pred_labels.csv", index=None)
