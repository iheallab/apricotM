...
if not os.path.exists(f"{MODEL_DIR}/apricott/results"):
    os.makedirs(f"{MODEL_DIR}/apricott/results")
...
model_architecture = torch.load(
    f"{MODEL_DIR}/apricott/model_architecture.pth"
)
...
model.load_state_dict(torch.load(f"{MODEL_DIR}/apricott/model_weights.pth", map_location=DEVICE))
...
with open("%s/apricott/best_params.pkl" % MODEL_DIR, "rb") as f:
...
for cohort in cohorts:
    ...
    true_labels.to_csv(
        f"{MODEL_DIR}/apricott/results/{cohort[1]}_true_labels.csv", index=None
    )
    pred_labels.to_csv(
        f"{MODEL_DIR}/apricott/results/{cohort[1]}_pred_labels.csv", index=None
    )
