...
from variables import time_window, PROSP_DATA_DIR, MODEL_DIR, VAR_MAP, ANALYSIS_DIR, SCALERS_DIR
...
with open(f"{SCALERS_DIR}/scalers_static.pkl", "rb") as f:
...
with open(f"{SCALERS_DIR}/scalers_seq.pkl", "rb") as f:
    scalers = pickle.load(f)
...
if cum_seq:
    ...
    outcomes["seq_len_sum"] = outcomes.groupby("icustay_id")["seq_len"].cumsum()

    seq_len = outcomes["seq_len_sum"].tolist()
    seq_len_idx = outcomes["seq_len"].tolist()

    icu_ids = seq.drop_duplicates(subset=["shift_id"], keep="first")
    icu_ids = icu_ids["icustay_id"].values

    seq.drop(columns="shift_id", inplace=True)

    seq = seq[["hours", "variable_code", "value", "icustay_id"]]
    icuid_idx = 3
    ...
    icu_len = 0
    idx_id = 0
    prev_id = X[0,icuid_idx]

    start = 0
    for i in range(len(outcomes)):
        
        curr_id = X[idx_id, icuid_idx]
        ...
        if seq_len[i] <= max_seq_len:
            ...
            qkv[i, : seq_len[i], 0] = X[start:end, 0]
            qkv[i, : seq_len[i], 1] = X[start:end, 1]
            qkv[i, : seq_len[i], 2] = X[start:end, 2]
            qkv[i, : seq_len[i], 3] = X[start:end, 3]

        else:
            end = start + seq_len[i]
            trunc = end - max_seq_len
            qkv[i, :, 0] = X[trunc:end, 0]
            qkv[i, :, 1] = X[trunc:end, 1]
            qkv[i, :, 2] = X[trunc:end, 2]
            qkv[i, :, 3] = X[trunc:end, 3]


        prev_id = X[idx_id, icuid_idx]
        idx_id += seq_len_idx[i]
...
with h5py.File(f"{ANALYSIS_DIR}/dataset_prospective.h5", "w", libver="latest") as hf:
