...
with open("%s/apricotm/best_params.pkl" % MODEL_DIR, "rb") as f:
...
for cohort in cohorts:
    ...
    true_labels.to_csv(
        f"{MODEL_DIR}/apricotm/results/{cohort[1]}_true_labels.csv", index=None
    )
    pred_labels.to_csv(
        f"{MODEL_DIR}/apricotm/results/{cohort[1]}_pred_labels.csv", index=None
    )
